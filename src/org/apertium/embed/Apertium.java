...
public class Apertium {
    public static final String DATA_DIRECTORY = "/sdcard/forapertium/";
    ...
    private static boolean _parseCommandLine(Apertium.InputOutput clp, String input, String modes) throws Exception {
	...
	private static void _dispatchPipeline(Mode mode,
            Apertium.InputOutput clp) throws Exception {
...
//        input = new StringReader("Saluton, mi estas Jacob");
    ...
    private static void _listModes(Apertium.InputOutput clp) {
     ...
     * @throws Exception
     */
    public static String exe(String input, String modes)
            throws Exception {
        //Ensure we are using UTF-8 encoding by default
        System.setProperty("file.encoding", "UTF-8");

        Apertium.InputOutput aio = new Apertium.InputOutput();

        if (!_parseCommandLine(aio, input, modes)) {
            return "1";
        }
        ...
        try {
            mode = new Mode(aio.dataDir + "modes/" + aio.direction + ".mode");
        } catch (IOException e) {
        ...
        _dispatchPipeline(mode, aio);
        try {
            aio.extOutput.flush(); //Just to make sure it gets flushed.
        } catch (IOException e) {
